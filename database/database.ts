...
import { ApiKeyModel, CourseModel, CredentialModel, ProjectModel, UserModel } from './models';
...
export const load = async () => {
	...
	try {
		...
		await ProjectModel.deleteMany().session(session);
		await ApiKeyModel.deleteMany().session(session);
